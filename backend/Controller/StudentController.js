...
module.exports.postStudent = async function postStudent(req,res){
    let pdata = req.body
    // console.log(pdata);
    const a = await StudentModel.find({Email:pdata.Email}).toArray();
    if(a.length){
        res.status(401).json({success:false,messsage:"Email ID already exists!"});
        return;
    }
    try{
        ...
        if(data){
            ...
            res.status(401).json({success:false,message:"Student is not Registered Successfully!!"})     
        }
    }
    catch(error){
        console.log(error);
...
module.exports.verifyProfile = async(req,res)=>{
    ...
    if(email){
        if(req.body.sendMail){
            const u = await StudentModel.find({email:email}).toArray();
            if(u.length==0){
                res.status(401).json({success:false,message:"No user registered!"});
                return;
            }
