import React,{useContext} from 'react'
import JobContext from '../context/jobcontext/JobContext';
import {useEffect} from 'react';
const LoginS = () => {
  const {loginStudent,load,setLoad} = useContext(JobContext);
   let handleLogin = async function login(e){
    e.preventDefault();
    setLoad(true);
    let Email = document.getElementById('email').value;
    let Password = document.getElementById('password').value;
    loginStudent({Email,Password});
  }
  useEffect(()=>{
      document.getElementById("box").addEventListener('keydown',(event)=>{
        if(event.key==="Enter"){
          document.getElementById("logins-btn").click();
        }
      })
  },[]);
  return (
    <div id="box" className="card container my-5 shadow-lg p-3 mb-5 bg-light rounded" style={{width: "25rem"}}>
      <div className="card-body">
      <h2 className="card-title">Student Login</h2>
      <div className="form-floating mb-3 ">
    <input type="email" className="form-control" id="email" placeholder="name@example.com"/>
    <label for="email">Email address</label>
  </div>
  <div className="form-floating mb-3">
    <input type="password" className="form-control" id="password" placeholder="Password"/>
    <label for="password">Password</label>
  </div>
  <div id="logins-btn" className='btn btn-outline-info my-3' style={{marginTop:"5rem"}} onClick={handleLogin}>
  {load?<span class="spinner-border spinner-border-sm" style={{marginRight:5}} role="status" aria-hidden="true"></span>:<></>}
  {load?"Loading":"Login"}</div>
  </div>
  </div>
  )
}

export default LoginS